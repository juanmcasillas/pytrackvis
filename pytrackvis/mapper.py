...
from .rainbowvis import Rainbow
...
class GenericMapper:
    ...
    def tile2url(tile, zoom):
        ...
        url = "https://otile%d.mqcdn.com/tiles/1.0.0/osm/%d/%d/%d.png" % (servernum, zoom, tile[0], tile[1])
        return url
...
class OSMMapper(GenericMapper):
    ...
    def GetMapCentered(self,center,radius,mapempty=False,zoom_base=None, bounding_box=True):
        ...
        NW = gpxpy.geo.Location(center.latitude, center.longitude)
        NE = gpxpy.geo.Location(center.latitude, center.longitude)
        SE = gpxpy.geo.Location(center.latitude, center.longitude)
        SW = gpxpy.geo.Location(center.latitude, center.longitude)

        NW.move(gpxpy.geo.LocationDelta(distance=radius, angle=270+45))
        NE.move(gpxpy.geo.LocationDelta(distance=radius, angle=0+45))
        SE.move(gpxpy.geo.LocationDelta(distance=radius, angle=90+45))
        SW.move(gpxpy.geo.LocationDelta(distance=radius, angle=180+45))
        ...
        # min_lat, min_lon SE.latitude, SW.longitude
        # max_lat, max_lon NW.latitude, NE.longitude
        min_pos = [SE.latitude, SW.longitude]
        max_pos = [NW.latitude, NE.longitude] 
        bb = [ min_pos, max_pos ]
        return self.GetMapBB( bb, mapempty, zoom_base,bounding_box)
    ...
    def _offset_meters(self, Origin, Point):
        ...
        y = math.fabs(y)
        x = math.fabs(x)
        return Size(x *gpxpy.geo.ONE_DEGREE, y*gpxpy.geo.ONE_DEGREE)
    ...
    def ProjectPoints(self, 
        ...
        for i in range(len(points)-1):
            ...
            if use_gradient and gradient_value:
                if gradient_value in dir(a):
                    gradient_val = getattr(a,gradient_value, 0)
                else:
                    use_gradient = False
                    gradient_value = None
            else:
            ...
            if ai in range(0,self.img_sz.width) and aj in range(0,self.img_sz.height) and\
               bi in range(0,self.img_sz.width) and bj in range(0,self.img_sz.height):
                ...
                if use_gradient:
                    draw.line( (ai,aj, bi, bj), fill="#%s" % rainbow.colourAt(gradient_val), width=width)
                else:
                    draw.line( (ai,aj, bi, bj), fill=color, width=width)
                del draw
                #mapimg.imagemap.putpixel((i,j),color)
    ...
    def ProjectArrows(self, length, points, mapimg, color=(200,100,100), width=3):
        ...
        while d < dist and k < len(points):
            d += gpxpy.geo.length_2d([points[k-1], points[k]])
            k += 1
